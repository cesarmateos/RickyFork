...
void hilolay_init(){
	...
	Logger_CreateLog("HILOLAY.log","hilolay",true);
	int socket = SocketClient_ConnectToServer("3801");
}

int suse_join(int hilo){
	SocketCommons_SendData(socket,BLOQUEAR,hilo,sizeof(int));
}
int suse_schedule_next(){
	SocketCommons_SendData(socket,PROXIMOHILOAEJECUTAR,NULL,sizeof(int));
}
int suse_create(int hiloAcargar){
	SocketCommons_SendData(socket,CARGARHILO,hiloAcargar,sizeof(int));
	return hiloAcargar;
}
int suse_close(int hiloAeliminar){
		SocketCommons_SendData(socket,ELIMINAR,hiloAeliminar,sizeof(int));*/
